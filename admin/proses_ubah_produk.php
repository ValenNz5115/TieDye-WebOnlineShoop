    ...
    if ($_POST) {
        ...
        if($file_size < 2048000 and ($file_type == "image/jpeg" or $file_type== "image/png")){
            ...
            if (empty($file_name)){
                ...
                $update= mysqli_query ($conn, "update produk set nama_produk='".$nama_produk."', deskripsi='".$deskripsi."', harga='".$harga."', foto_produk='".$file_name."' where id_produk='".$id_produk."' ") or die (mysqli_error($conn));
                if ($update) {
                    echo "<script> alert ('Sukses update produk'); location.href='data_produk.php' ; </script>";  
                }else{
                    echo "<script> alert ('Gagal update produk'); location.href='data_produk.php' ; </script>";
