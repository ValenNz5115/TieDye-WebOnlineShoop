        ...
        <div class='card-body'>
            <table class="table table-bordered fs-5 fw-light text-center">
                ...
                if (isset($_POST["cari"])) {
                    ...
                    $cari=$_POST['cari'];
                    $query_transaksi= mysqli_query($conn,"SELECT * from transaksi join pelanggan on pelanggan.id_pelanggan= transaksi. id_pelanggan join detail_transaksi on detail_transaksi. id_transaksi=transaksi.id_transaksi join produk on produk. id_produk= detail_transaksi.id_produk where pelanggan.nama_pelanggan LIKE nama_pelanggan or produk.nama_produk LIKE nama_produk group by nama_produk ORDER BY id_detail_transaksi DESC");
                }else{
                ...
                while($data_transaksi= mysqli_fetch_array($query_transaksi)) {
                    $no++;
                    $produk_dibeli = "<ol>";
                            $qry_produk = mysqli_query($conn,"select * from  detail_transaksi join produk on produk.id_produk=detail_transaksi.id_produk where id_transaksi = ".$data_transaksi['id_transaksi']);
                            while($dt_produk=mysqli_fetch_array($qry_produk)){ //perulangan untuk menampilkan detail transaksi dan subtotalnmya
                                $produk_dibeli .= "<li class = 'produk'>".$dt_produk['nama_produk']."</li>";
                            }
                            $produk_dibeli.="</ol>";
                    ?>
                    <tr>
